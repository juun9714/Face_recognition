...
for i in range(1,4):
    vidcap = cv2.VideoCapture('./video_files/episode%d.avi' % i)
    count = 1 # 추출 이미지 개수 카운트
    real_count=1
    os.mkdir("./image_files/episode%d"%i)  # 폴더 생성
            ...
            for (x,y,w,h) in faces:
                ...
                if flag==1:
                    if count%15==0:
                        ...
                        cv2.imwrite("./image_files/episode{}/cropped_{}.jpg".format(i, real_count),cropped) # 추출된 이미지가 저장되는 경로와 파일명을 지정.
                        real_count+=1
